...
import { CalendarClock, Copy, Link2, MousePointerClick, ShoppingBag, TimerReset, Wallet } from 'lucide-react';
import { motion } from 'framer-motion';
import { toast } from 'sonner';
import { AffiliateLink, AffiliateSummary } from '@/types';
import { getMyAffiliateLinks, getAffiliateSummary } from '@/lib/api';
...
const defaultSummary: AffiliateSummary = {
  totalEarned: 0,
  totalWithdrawn: 0,
  available: 0,
  minWithdraw: 0,
...
const Affiliates = () => {
  ...
  const [summary, setSummary] = useState<AffiliateSummary>(defaultSummary);
  ...
  const totalOrders = links.reduce((sum, link) => sum + (link.ordersCount || 0), 0);
  const totalLinks = summary.totalLinks || links.length;
  const expiredLinks = Math.max(totalLinks - (summary.activeLinks || 0), 0);
  ...
  return (
    <Layout>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="mb-8 grid grid-cols-2 gap-3 lg:grid-cols-7">
          {showSkeleton ? (
            Array.from({ length: 7 }).map((_, index) => <StatSkeleton key={`stat-skeleton-${index}`} />)
          ) : (
            [
              { label: 'Links Gerados', value: String(totalLinks), icon: Link2, color: 'text-primary' },
              { label: 'Links este mes', value: `${summary.linksCreatedThisMonth ?? 0}/${summary.monthlyLinkLimit ?? 3}`, icon: TimerReset, color: 'text-info' },
              { label: 'Links Ativos', value: String(summary.activeLinks ?? 0), icon: CalendarClock, color: 'text-success' },
        ...
        {!showSkeleton && (
          <div className="mb-4 rounded-xl border border-border bg-card/70 p-4 text-sm text-muted-foreground shadow-card">
            <div className="flex flex-wrap items-center gap-3">
              <span>Links Ativos: <strong className="text-foreground">{summary.activeLinks}</strong></span>
              <span>Links Expirados: <strong className="text-foreground">{expiredLinks}</strong></span>
            </div>
          </div>
        )}
        ...
        <div className="space-y-4">
          {showSkeleton ? (
            Array.from({ length: 3 }).map((_, index) => <LinkSkeleton key={`link-skeleton-${index}`} />)
          ) : pagedLinks.length > 0 ? (
            <>
              {pagedLinks.map((link, index) => (
                ...
                >
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`rounded-md px-2 py-0.5 text-xs font-medium ${
                            link.isExpired ? 'bg-destructive/10 text-destructive' : 'bg-success/10 text-success'
                          }`}
                        >
                          {link.isExpired ? 'Expirado' : 'Ativo'}
                        </span>
                      </div>
                    </div>
                    <div className="text-right">
                  ...
                  <div className="flex flex-wrap gap-4 border-t border-border pt-3 text-xs text-muted-foreground">
                    ...
                    <span>Expira em: {link.expiresAt ? new Date(link.expiresAt).toLocaleDateString('pt-BR') : '-'}</span>
                  </div>
